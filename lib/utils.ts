...
export const setupRecording = (
    ...
    handlers: RecordingHandlers
        ...
        // First, try the preferred configuration
    if (MediaRecorder.isTypeSupported(DEFAULT_RECORDING_CONFIG.mimeType)) {
        console.log(`Using preferred mimeType: ${DEFAULT_RECORDING_CONFIG.mimeType}`);
    ...
    // If not supported, try a more common fallback without specific codecs
    console.warn(`${DEFAULT_RECORDING_CONFIG.mimeType} is not supported. Trying video/webm.`);
    if (MediaRecorder.isTypeSupported('video/webm')) {
        console.log("Using fallback mimeType: video/webm");
    ...
    // If that also fails, let the browser decide by providing no options
    console.warn('video/webm is not supported. Letting the browser choose the default.');
...
export const createIframeLink = (videoId: string) =>
  ` https://iframe.mediadelivery.net/embed/485760/${videoId}?autoplay=false&preload=true`;
